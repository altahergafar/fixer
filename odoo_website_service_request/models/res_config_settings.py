...
class ResConfigSettings(models.TransientModel):
    ...
    def _onchange_closed_stage_id(self):
        ...
        if self.closed_stage_id:
            ...
            in_stage = self.env['service.request.stage'].search([('id', '=', stage)])
            not_in_stage = self.env['service.request.stage'].search(
                [('id', '!=', stage)])
    ...
    def _constrains_show_category_subcategory(self):
        ...
        if self.show_category:
            group_cat = self.env.ref(
                'odoo_website_service_request.group_show_category')
            group_cat.write({
