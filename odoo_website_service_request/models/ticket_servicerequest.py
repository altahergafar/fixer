...
class ServiceRequest(models.Model):
    ...
    _description = 'servicerequest Ticket'
    ...
    ticket_type_id = fields.Many2one('servicerequest.type',
    ...
    def auto_close_ticket(self):
        ...
        auto_close = self.env['ir.config_parameter'].sudo().get_param(
            'odoo_website_service_request.auto_close_ticket')
        if auto_close:
    ...
    def action_open_merged_tickets(self):
        ...
        ticket_ids = self.env['support.servicerequest'].search(
            [('merged_ticket', '=', self.id)])
        servicerequest_ticket_ids = ticket_ids.mapped('display_name')
        help_ticket_records = self.env['service.request'].search(
            [('name', 'in', servicerequest_ticket_ids)])
        return {
            'type': 'ir.actions.act_window',
            'name': 'servicerequest Ticket',
            'view_mode': 'tree,form',
