...
class TicketPortal(portal.CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        ...
        if 'ticket_count' in counters:
            ticket_count = request.env['service.request'].search_count(
    ...
    def portal_my_tickets(self):
        ...
        tickets = request.env['service.request'].sudo().search(domain)
        values = {
            'default_url': "/my/tickets",
            'tickets': tickets,
            'page_name': 'ticket',
        }
        return request.render("odoo_website_service_request.portal_my_tickets", values)
    ...
    def portal_tickets_details(self, **kwargs):
        ...
        details = request.env['service.request'].sudo().browse(ticket_id)
        ...
        return request.render("odoo_website_service_request.portal_ticket_details",
                              data)
    ...
    def ticket_download_portal(self, **kwargs):
        ...
        data = {
            'help': request.env['service.request'].sudo().browse(ticket_id)}
        report = request.env.ref(
            'odoo_website_service_request.report_ticket')
        pdf, _ = report.sudo()._render_qweb_pdf(
            report, res_ids=ticket_id, data=data)
        pdf_http_headers = [('Content-Type', 'application/pdf'),
                            ('Content-Length', len(pdf)),
                            ('Content-Disposition',
                             'attachment; filename="servicerequest Ticket.pdf"')]
